...
export default async function TransactionsDashboard({ searchParams }: { searchParams: Promise<any> }) {
  ...
  const errorObj = txData?.error || coinsData?.error || rewardsData?.error || catData?.error || monthData?.error;
  ...
  return (
    <main className="dashboard-container">
      ...
      {errorObj ? (
        ...
        <>
          {rewardsData && coinsData && !rewardsData.error && (
            <RewardsCatalogue rewards={rewardsData} initialBalance={coinsData.coin_balance} />
          )}

          <div className="charts-row">
            {catData && !catData.error && <CategoryChart data={catData} />}
            {monthData && !monthData.error && <MonthlyChart data={monthData} />}
          </div>
